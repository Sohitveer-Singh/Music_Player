...
let masterbackw = document.getElementById("previousSong");
let masterforw = document.getElementById("nextSong");
let timer = document.getElementById("time");
let myprogressBar = document.getElementById("bar1");

let songPlayOrder = new Array();
...
function varinderAbout() {
  ...
  aboutSection.style.zIndex = "2";
  aboutArtist.innerHTML =
    `Varinder Brar is a popular Punjabi singer and music composer who has been active in the industry for over 15 years. He is known for his melodic voice, which has made him a fan favorite in the Punjabi music industry. He is known for his collaborations with several artists such as Roshan Prince, Sukh-E Musical Doctorz, and Gippy Grewal. ` +
    "<br />" +
...
function parmishAbout() {
  ...
  aboutSection.style.zIndex = "2";
  aboutArtist.innerHTML =
    `Parmish Verma (born 3 July 1990) is an Punjabi singer, rapper, music video director, videographer and actor associated with the Punjabi Music and Punjabi Film Industry. He started his career as a video director then singer, and later debuted as an actor with the film Rocky Mental. ` +
    "<br />" +
...
function Playlist() {
  ...
  masterbackw.addEventListener("click", () => {
    if(i>0){
      i = i-1;
      audio.src = playlist[i];
      currentAudio.play();
  ...
  masterforw.addEventListener("click", () => {
    if(i<15){
      i = i+1;
      audio.src = playlist[i];
      currentAudio.play();
